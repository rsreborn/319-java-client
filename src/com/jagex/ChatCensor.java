...
public class ChatCensor {
    ...
    public static void load(CacheArchive cacheArchive) {
        ...
        Buffer badEncBuffer = new Buffer(cacheArchive.readFile("badenc.txt", null));
        Buffer domainEncBuffer = new Buffer(cacheArchive.readFile("domainenc.txt", null));
        Buffer topLevelDomainsBuffer = new Buffer(cacheArchive.readFile("tldlist.txt", null));
        method532(fragmentsEncBuffer, badEncBuffer, domainEncBuffer, topLevelDomainsBuffer);
    }

    public static void method532(Buffer fragmentsEncBuffer, Buffer badEncBuffer, Buffer domainEncBuffer, Buffer topLevelDomainsBuffer) {
        method534(badEncBuffer);
        method535(domainEncBuffer);
        method536(fragmentsEncBuffer);
        method533(topLevelDomainsBuffer);
    }
    ...
    public static void method539(char[] ac) {
        int j = 0;
        for (int k = 0; k < ac.length; k++) {
            if (method540(ac[k])) {
                ac[j] = ac[k];
    ...
    public static boolean method540(char c) {
        return c >= ' ' && c <= '\177' || c == ' ' || c == '\n' || c == '\t' || c == '\243' || c == '\u20AC';
    }
    ...
    public static void method543(char[] ac) {
        boolean flag = true;
    ...
    public static void method544(char[] ac) {
        for (int i = 0; i < 2; i++) {
            for (int j = aCharArrayArray709.length - 1; j >= 0; j--) {
                method553(ac, aByteArrayArrayArray710[j], aCharArrayArray709[j]);
            }
        }
    }

    public static void method545(char[] ac) {
        char[] ac1 = ac.clone();
        char[] ac2 = {
                '(', 'a', ')'
        };
        method553(ac1, null, ac2);
        char[] ac3 = ac.clone();
        char[] ac4 = {
                'd', 'o', 't'
        };

        method553(ac3, null, ac4);
        for (int j = aCharArrayArray711.length - 1; j >= 0; j--) {
            method546(ac3, ac, aCharArrayArray711[j], ac1);
        }
    }

    public static void method546(char[] ac, char[] ac1, char[] ac2, char[] ac3) {
        ...
        for (int j = 0; j <= ac1.length - ac2.length; j += i) {
            ...
            if (l >= ac2.length) {
                boolean flag2 = false;
                int j1 = method547(ac1, j, ac3);
                int k1 = method548(ac1, k - 1, ac);
                if (j1 > 2 || k1 > 2) {
                    flag2 = true;
                }
                if (flag2) {
    ...
    public static int method547(char[] ac, int i, char[] ac1) {
        if (i == 0) {
            return 2;
        }
        for (int j = i - 1; j >= 0; j--) {
    ...
    public static void method549(char[] ac) {
        ...
        for (int k = 0; k < aCharArrayArray712.length; k++) {
            method550(anIntArray713[k], ac3, aCharArrayArray712[k], ac1, ac);
        }
    }

    public static void method550(int i, char[] ac, char[] ac1, char[] ac2, char[] ac3) {
    ...
    public static int method551(char[] ac, char[] ac1, int i) {
        if (i == 0) {
            return 2;
        }
        for (int j = i - 1; j >= 0; j--) {
