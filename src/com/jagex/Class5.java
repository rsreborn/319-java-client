package com.jagex;

public class Class5 {

    public static int[] anIntArray105;
    public int anInt72 = 4096;
    public int anInt73 = 16;
    public int anInt74 = 258;
    public int anInt75 = 23;
    public int anInt76 = 1;
    public int anInt77 = 6;
    public int anInt78 = 50;
    public int anInt79 = 4;
    public int anInt80 = 18002;
    public byte[] aByteArray81;
    public int anInt82;
    public int anInt83;
    public int anInt84;
    public int anInt85;
    public byte[] aByteArray86;
    public int anInt87;
    public int anInt88;
    public int anInt89;
    public int anInt90;
    public byte aByte91;
    public int anInt92;
    public boolean aBoolean93;
    public int anInt94;
    public int anInt95;
    public int anInt96;
    public int anInt97;
    public int anInt98;
    public int anInt99;
    public int anInt100;
    public int[] anIntArray101;
    public int anInt102;
    public int[] anIntArray103;
    public int[] anIntArray104;
    public int anInt106;
    public boolean[] aBooleanArray107;
    public boolean[] aBooleanArray108;
    public byte[] aByteArray109;
    public byte[] aByteArray110;
    public int[] anIntArray111;
    public byte[] aByteArray112;
    public byte[] aByteArray113;
    public byte[][] aByteArrayArray114;
    public int[][] anIntArrayArray115;
    public int[][] anIntArrayArray116;
    public int[][] anIntArrayArray117;
    public int[] anIntArray118;
    public int anInt119;

    public Class5() {
        anInt72 = 4096;
        anInt73 = 16;
        anInt74 = 258;
        anInt75 = 23;
        anInt76 = 1;
        anInt77 = 6;
        anInt78 = 50;
        anInt79 = 4;
        anInt80 = 18002;
        anIntArray101 = new int[256];
        anIntArray103 = new int[257];
        anIntArray104 = new int[257];
        aBooleanArray107 = new boolean[256];
        aBooleanArray108 = new boolean[16];
        aByteArray109 = new byte[256];
        aByteArray110 = new byte[4096];
        anIntArray111 = new int[16];
        aByteArray112 = new byte[18002];
        aByteArray113 = new byte[18002];
        aByteArrayArray114 = new byte[6][258];
        anIntArrayArray115 = new int[6][258];
        anIntArrayArray116 = new int[6][258];
        anIntArrayArray117 = new int[6][258];
        anIntArray118 = new int[6];
    }
}
